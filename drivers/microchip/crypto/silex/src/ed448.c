 ...
 */
static inline void write_ed448dgst(const struct sx_ed448_dgst *op,
...
struct sx_pk_acq_req sx_async_ed448_ptmult_go(struct sx_pk_cnx *cnx,
   const struct sx_ed448_dgst *r)
{
   struct sx_pk_acq_req pkreq;
   const struct sx_pk_ecurve curve = sx_pk_get_curve_ed448(cnx);
...
int sx_ed448_ptmult(struct sx_pk_cnx *cnx,
   const struct sx_ed448_dgst *r, struct sx_ed448_pt *pt)
{
   struct sx_pk_acq_req pkreq;
   uint32_t status;

   pkreq = sx_async_ed448_ptmult_go(cnx, r);
   if (pkreq.status)
      return pkreq.status;
...
struct sx_pk_acq_req  sx_pk_async_ed448_sign_go(
      struct sx_pk_cnx *cnx,
      const struct sx_ed448_dgst *k,
      const struct sx_ed448_dgst *r,
...
int sx_ed448_sign(struct sx_pk_cnx *cnx,
                    const struct sx_ed448_dgst *k,
                    const struct sx_ed448_dgst *r,
                    const struct sx_ed448_v *s,
                    struct sx_ed448_v *sig_s)
{
   struct sx_pk_acq_req pkreq;
   uint32_t status;

   pkreq = sx_pk_async_ed448_sign_go(cnx, k, r, s);
   if (pkreq.status)
      return pkreq.status;
   status = sx_pk_wait(pkreq.req);
...
static inline void ed448_pt_write_y(const struct sx_ed448_pt *pt, char *ay)
{
   ...
   ay[SX_ED448_PT_SZ-1] = pt->encoded[SX_ED448_PT_SZ-1] & 0x7f;
}


struct sx_pk_acq_req sx_async_ed448_verify_go(struct sx_pk_cnx *cnx,
   const struct sx_ed448_dgst *k,
...
int sx_ed448_verify(struct sx_pk_cnx *cnx,
                      const struct sx_ed448_dgst *k,
                      const struct sx_ed448_pt *a,
                      ...
                      const struct sx_ed448_pt *r)
{
   ...
   pkreq = sx_async_ed448_verify_go(cnx, k, a, sig_s, r);
   if (pkreq.status)
      return pkreq.status;
   status = sx_pk_wait(pkreq.req);
