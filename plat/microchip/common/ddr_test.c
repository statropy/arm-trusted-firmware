...
uintptr_t ddr_test_addr_bus(struct ddr_config *config, uintptr_t ddr_base_addr, bool cache)
{
	...
	INFO("DDR addr bus begin, start %08lx, size 0x%08x, cache %d\n",
	     ddr_base_addr, config->info.size, cache);

	if (cache)
		inv_dcache_range(ddr_base_addr, config->info.size);

	/* Write the default pattern at each of the power-of-two offsets. */
	for (offset = sizeof(uint32_t); offset < config->info.size; offset <<= 1U) {
	...
	for (testoffset = sizeof(uint32_t); testoffset < config->info.size; testoffset <<= 1U) {
		mmio_write_32(ddr_base_addr + testoffset, DDR_PATTERN2);
	...
	if (cache)
		clean_dcache_range(ddr_base_addr, config->info.size);
...
uintptr_t ddr_test_rnd(struct ddr_config *config, uintptr_t ddr_base_addr, bool cache, uint32_t seed)
{
	...
	for (offset = 0, value = seed; offset < config->info.size; offset += sizeof(uint32_t)) {
		value = ps_rnd(value);
